...
60%-20%-20% for training/validation/testing within a single database
+---------------------------
before this, you should have already done with the code of feature extraction  
+---------------------------
`CUDA_VISIBLE_DEVICES=1 nohup python3 -u demo_run_main.py --database KoNViD > log.intra.KoNViD.out 2>&1 &`
"""
...
if __name__ == "__main__":
    parser = ArgumentParser(description='UGC-VQA based on Decomposition and Recomposition')
    ...
    base_path = path_ + database + '/'
    info_file = base_path + database + '_info.txt'
    _, mos = read_info(base_path, info_file)
    ...
    with open('./data/{}.{}.NT-{}.pkl'.format('dsd_pro_appearance', database, nT), 'rb') as f:
        features = pickle.load(f)     # [N, nt, 192*2+320*2+1088*2+1536*2]
    blk_list = [192 * 2, 320 * 2, 1088 * 2, 1536 * 2]

    with open('./data/{}.{}.NT-{}.pkl'.format('dsd_pro_motion', database, nT), 'rb') as f:
        sf_features = pickle.load(f)  # [N, nt, 32*2+64*2+128*2+256*2]

    features = np.concatenate([features[:, :, :192*2], sf_features[:, :, :32*2],
                               features[:, :, 192*2:(192+320)*2], sf_features[:, :, 32*2:(32+64)*2],
                               features[:, :, (192+320)*2:-1536*2], sf_features[:, :, (32+64)*2:-256*2],
                               features[:, :, -1536*2:], sf_features[:, :, -256*2:]], 2)
    ...
    for t in range(num_repeat):
        ...
        pos_train_end = int((1 - args.test_ratio - args.val_ratio) * num_samples)
        pos_eval_end = int((1 - args.test_ratio) * num_samples)
        trainindex = index[:pos_train_end]                 # the first 60%
        evalindex = index[pos_train_end:pos_eval_end]
        testindex = index[pos_eval_end:num_samples]        # the last 20%
        ...
        train_dataset = VQADataset(features[trainindex], mos[trainindex])
        train_loader = torch.utils.data.DataLoader(dataset=train_dataset,
                                                   batch_size=args.batch_size,
                                                   shuffle=True, drop_last=True)
        val_dataset = VQADataset(features[evalindex], mos[evalindex])
        val_loader = torch.utils.data.DataLoader(dataset=val_dataset)
        
        test_dataset = VQADataset(features[testindex], mos[testindex])
        ...
        print('Training/Validation/Testing: {}/{}/{}'.format(num_train, num_eval, num_test))
        ...
        with torch.no_grad():
            ...
            print('-----' * 5)
            print("Intra-Test (6/2/2): SROCC={:.4f}, KROCC={:.4f}, PLCC={:.4f}, RMSE={:.4f}"
                    .format(test_SROCC, test_KROCC, test_PLCC, test_RMSE))
    ...
    print('MEDIAN performance: SROCC/KROCC/PLCC/RMSE: {}'.format(np.median(perf, 0)))  # adopted as performance
