...
class SlowFast(nn.Module):
    ...
    def forward(self, input):
        # fast, fast_std, lateral = self.FastPath(input[:, :, ::1, :, :])
        # slow, slow_std = self.SlowPath(input[:, :, ::4, :, :], lateral)
        # sf_ave = torch.cat([slow, fast], 1)
        # sf_std = torch.cat([slow_std, fast_std], 1)
        # return torch.cat([sf_ave, sf_std], 1)
    ...
    def SlowPath(self, input, lateral):
        ...
        x = self.slow_res5(x)
        xm = nn.AdaptiveAvgPool3d(1)(x)
        xm = xm.view(-1, xm.size(1))

        xs = torch.std(x.flatten(2), 2)
        return xm, xs
    ...
    def FastPath(self, input):
        ...
        res4 = res4.flatten(2)
        res5 = res5.flatten(2)
        f = torch.cat([torch.mean(res2, dim=2, keepdim=False), torch.std(res2, dim=2, keepdim=False),
                       torch.mean(res3, dim=2, keepdim=False), torch.std(res3, dim=2, keepdim=False),
                       torch.mean(res4, dim=2, keepdim=False), torch.std(res4, dim=2, keepdim=False),
                       torch.mean(res5, dim=2, keepdim=False), torch.std(res5, dim=2, keepdim=False)], 1)
...
def sfresnet50(pretrained=False, **kwargs):
...
if __name__ == '__main__':
    ...
    height = 216
    width = 384
    x = torch.rand(batch, ch, T, height, width).float()
